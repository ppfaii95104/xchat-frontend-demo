...
import { tagSample } from "@/app/chat/sampleData";

export default function SearchTagProfileComponents(_prop: any) {
  const [form] = Form.useForm();
  const [listTag, setListTag] = useState<tagType[]>(tagSample);
  const [openTagModal, setOpenTagModal] = useState(false);
  const onFinish: FormProps<FieldType>["onFinish"] = (values: any) => {
    if (_.isEmpty(values.search)) {
      setListTag({ id: _prop?.listTag.length, ..._prop?.listTag });
    } else {
      const filter = _.filter(_prop?.listTag, (a) => a.name === values.search);
      setListTag(filter);
    }
  };
  const selectTag = async (values: any) => {
    const newData = _prop?.listTag;
    _prop?.setListTag([]);
            ...
            {_.isEmpty(listTag) ? (
              <div>
                <div className="flex justify-between items-center">
                  <span className="font-noto text-grey-lable">
                    {form.getFieldValue("search")}
                  </span>
                  <Button
                    type="link"
                    className="flex items-center p-0 font-noto text-primary"
              ...
              <>
                {_.map(listTag, (item: tagType) => {
                  return (
                    <Button
                      className="flex items-center text-drak-grey p-2 font-noto w-full justify-between border-0 mb-2"
                      onClick={() => {
                        selectTag(item);
                      }}
                      style={{
